...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, share_audio_net):    
    ...
    if not share_audio_net:
        motion_net = MotionNetwork(args=dataset).cuda() # 모션 네트워크 초기화. 여기서 초기화되는 네트워크는 UMF이고, gaussians 객체 내부에 PMF가 존재. (PMF를 가우시안 객체 안에 넣어두는 이유는, 각 인물별 PMF가 다르기 때문이다!)
    ...
    for iteration in range(first_iter, opt.iterations + 1): # 1부터 30001까지 반복.
        ...
        # 이는 입 영역을 손실 계산에서 제외하기 위함이다. 즉, 해당 훈련 코드에서는 입 부분은 완전히 마스킹 하고 얼굴 부분만 생성한다.
        ...
        if iteration > warm_step: # 학습 중반 (1000 iter 이후)
            ...
            if iteration > p_motion_start_iter:
                ...
                p_motion_preds = gaussians.neural_motion_grid(gaussians.get_xyz, audio_feat, exp_feat) # 마크롱 `gaussians` 객체의 PMF 모델을 사용하여 개인화된 모션 예측(`p_motion_preds`)을 수행한다.
                # # `gaussians.get_xyz`(가우시안의 XYZ 좌표), `audio_feat`, `exp_feat`를 입력으로 사용한다.
                contrast_loss = 0 # 비교 손실을 0으로 초기화

                for tmp_scene_idx in range(len(scene_list)): # 마크롱, 오바마, 문제인에 대한 scene을 순차적으로 순회. (한 번 하고 마는 게 아니라, 모든 인물과 비교)
                    if tmp_scene_idx == cur_scene_idx: continue # 마크롱을 마크롱과 비교할 수는 없으니 건너 뛰기.
                    with torch.no_grad(): # 예시를 들어 오바마와 비교한다고 하자. 오바마의 PMF는 현재 훈련 대상이 아니므로 torch.no_grad()를 붙임.
                        tmp_scene = scene_list[tmp_scene_idx] # 오바마에 해당하는 scene을 가져옴
                        tmp_gaussians = tmp_scene.gaussians # 오바마의 가우시안 모델을 가져옴
                        tmp_p_motion_preds = tmp_gaussians.neural_motion_grid(gaussians.get_xyz, audio_feat, exp_feat) # 동일한 오디오와 표정 특징을 사용하여 오바마의 PMF 결과 파악 (이 때, 오바마의 PFM는 훈련되면 안 되니 위에 torch.no_grad()를 붙임)
            ...
            # `render_pkg` 딕셔너리에서 어텐션 맵(`attn`)의 1번 인덱스에 해당하는 입술 영역(`xmin:xmax, ymin:ymax`)의 평균 값에 $5 \times 10^{-3}$를 곱하여 손실에 추가한다.
            # 어텐션 맵에서 입술 영역의 부분만 잘라 로
        ...
        # 메모리 사용량을 줄이고 계산 속도를 높이며, 학습과는 무관한 부가 작업에서 모델 파라미터가 실수로 업데이트되는 것을 방지하기 위해 아래부터 그래디언트 추적 비활성화.
            ...
            if (iteration in checkpoint_iterations): # 현재 반복 횟수(`iteration`)가 `checkpoint_iterations` 리스트의 원소에 해당한다면
                ...
                # 이 컨텍스트 내에서는 `motion_net`의 파라미터들이 EMA(Exponential Moving Average) 값으로 일시적으로 대체
                    ...
                    # 그러나 `ckpt_ema` (EMA 기반의 체크포인트)가 아닌 `ckpt`를 동일한 이름으로 같이 저장하고 있다! 실험한 사람들이 굳이 EMA가 없어도 UMF가 잘 학습된다고 판단해서 그런 것 같은데, 내 코드 짤 때 이 부분은 검증을 해야겠다.
            ...
            # 이는 최종 반복 횟수에 도달하기 전까지만 옵티마이저 스텝을 수행하도록 한다.
                ...
                # `motion_optimizer`를 사용하여 `motion_net`의 파라미터들을 업데이트한다.
