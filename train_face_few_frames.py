...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, mode_long, pretrain_ckpt_path):
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        no_visible = False
        try:
            # visibility_filter: (N_points,) bool
            if visibility_filter is not None and visibility_filter.numel() > 0:
                if visibility_filter.sum().item() == 0:
                    no_visible = True
            else:
                # radii 모양으로도 확인 (뷰에 기여한 포인트가 0개면 radii[visibility_filter]가 길이 0)
                if radii is not None and radii.numel() == 0:
                    no_visible = True
        except Exception:
            pass
        ...
        # 완전 빈 프레임(이미지 합 0) 같은 경우도 보호
        if (image_white.abs().sum() + gt_image_white.abs().sum()) == 0:
            no_visible = True
        ...
        #         print(f"[warn] backward failed at iter {iteration}: {e}")
        #         # bad iteration skip
        #         optimizer.zero_grad()
        #         continue  # or break if too frequent
        # else:
        #     # print(f"[warn] iter {iteration}: no visible gaussians → skip backward")
        #     # 그래디언트만 초기화하고 스킵
        #     motion_optimizer.zero_grad()
        #     gaussians.optimizer.zero_grad(set_to_none=True)
        #     print("No visible gaussians → skip backward")
        #     # 다음으로 진행 (optimizer.step()은 아래 if에서 iteration<opt.iterations일 때만 호출되므로 영향 없음)
        #     # continue  # 원하면 아예 해당 이터레이션 건너뛰기
